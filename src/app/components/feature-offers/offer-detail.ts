...
@Component({
  ...
  template: `
            ...
            <div class="flex-1 overflow-y-auto p-12 bg-white flex flex-col items-center pt-20">
                <div class="w-full max-w-3xl bg-[#EEF2FF] rounded-xl p-12 text-center mb-10 shadow-sm border border-[#E0E7FF]">
                    ...
                    <div class="inline-flex items-center justify-center px-6 py-2 bg-white rounded-full shadow-sm text-[#313B85] font-bold text-sm border border-[#E0E7FF]">
                        Budget : {{ request()!.targetPrice > 0 ? formatPrice(request()!.targetPrice) : 'NC' }}
                    </div>
                </div>
        ...
        <div class="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-6 py-4 flex justify-between items-center z-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
             @if (isOwner()) {
               ...
               @if (offer()!.status === 'VENDU') {
                   ...
                   <button disabled class="px-6 py-2.5 rounded-md bg-gray-100 text-gray-400 font-bold text-sm cursor-not-allowed border border-gray-200 uppercase tracking-wide">Lot vendu</button>
               } 
               @else {
                   <div class="flex-1"></div>
        ...
        <div class="fixed inset-0 z-[50000] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh] animate-scale-in">
                ...
                <div class="p-8 overflow-y-auto space-y-8">
                    <div><h3 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">1. Identité & Prix</h3><div class="grid grid-cols-2 gap-4"><div class="col-span-2"><label class="block text-xs font-bold text-gray-700 mb-1">Nom</label><input [(ngModel)]="editFormData.name" type="text" class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm"></div><div><label class="block text-xs font-bold text-gray-700 mb-1">Millésime</label><input [(ngModel)]="editFormData.vintage" type="number" class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm"></div><div><label class="block text-xs font-bold text-gray-700 mb-1">Type</label><select [(ngModel)]="editFormData.productType" class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm"><option value="VIN ROUGE">VIN ROUGE</option><option value="VIN BLANC">VIN BLANC</option><option value="CHAMPAGNE">CHAMPAGNE</option><option value="SPIRITUEUX">SPIRITUEUX</option></select></div><div><label class="block text-xs font-bold text-gray-700 mb-1">Région</label><input [(ngModel)]="editFormData.region" type="text" class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm"></div><div><label class="block text-xs font-bold text-gray-700 mb-1">Prix Unitaire</label><input [(ngModel)]="editFormData.price.unit" type="number" class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm font-bold"></div></div></div>
                    </div>
    ...
    @if (showInterestModal()) { 
      <div class="fixed inset-0 z-[50000] flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-[420px] p-6 text-center animate-scale-in border border-gray-100">
          ...
          <h3 class="text-xl font-bold text-slate-900 mb-1">{{ interestType() === 'PRIMARY' ? 'Confirmer le positionnement' : 'Rejoindre la liste d\\'attente' }}</h3>
          ...
          <div class="space-y-3"><button (click)="confirmInterest()" class="w-full inline-flex justify-center rounded-lg px-4 py-3 text-sm font-bold text-white shadow-sm focus:outline-none transition-all active:scale-[0.98]" [ngClass]="interestType() === 'PRIMARY' ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-[#c2410c] hover:bg-[#9a3412]'">{{ interestType() === 'PRIMARY' ? 'Confirmer mon intérêt' : 'M\\'inscrire sur la liste' }}</button><button (click)="closeInterestModal()" class="w-full inline-flex justify-center rounded-lg bg-white px-4 py-3 text-sm font-bold text-gray-700 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 transition-colors">Annuler</button></div>
        </div>
      </div>
    }
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-[480px] p-6 animate-scale-in border border-gray-100">
            ...
            @if (request()) {
                ...
                <div class="flex justify-end gap-3">
                    <button (click)="closeContactModal()" class="px-5 py-2.5 rounded-lg border border-gray-300 text-gray-700 font-bold text-sm hover:bg-gray-50 transition-all">Annuler</button>
                    <button (click)="sendInquiry()" class="px-5 py-2.5 rounded-lg bg-slate-900 text-white font-bold text-sm hover:bg-slate-800 shadow-lg shadow-slate-900/20 transition-all">Confirmer</button>
                </div>
            } 
            @else {
    ...
    @if (showToast()) {
        <div class="fixed top-6 left-1/2 z-[100000] bg-slate-900 text-white px-4 py-2 rounded-full shadow-2xl flex items-center gap-3 border border-white/10" [ngClass]="isToastClosing() ? 'animate-toast-out' : 'animate-toast-in'">
    ...
    @if (isLightboxOpen()) { <div class="fixed inset-0 z-[9999] bg-black/95 flex items-center justify-center" (click)="closeLightbox()"><img [src]="currentPhoto()" class="max-h-[85vh]"></div> }
  `,
  styles: [`
